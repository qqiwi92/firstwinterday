...
void next(std::ostream &os, std::istream &is, size_t &ctx)
{
  size_t i = 0;
  is >> i;
  if (is.fail())
...
void last(std::ostream &os, std::istream &is, size_t &ctx)
{
  os << "<" << ctx <<"!>\n";
}
...
size_t match(const char *word, const char *const *words, size_t k)
{
  ...
  {
    if (std::strlen(word) == std::strlen(words[i]) &&
        !std::strcmp(word, words[i]))
    {
      return i;
    }
  }
...
int main()
{
  ...
  void (*cmds[cmds_count])(std::ostream &os, std::istream &is,
                           size_t &) = {next, last};
